...
<div id="content">
	<h2><?php echo L('use_mobile_get_password');?></h2>
	<?php if($step==1) { ?>
...
//-->
	...
	<form method="post" action="" id="myform">
		<div class="col-left form-login form-reg">
			<div class="point">
				<div class="content">
					<strong class="title">步骤：1. 用户名确认 &nbsp;&nbsp;>&nbsp;&nbsp; 2. 手机号码确认 &nbsp;&nbsp;>&nbsp;&nbsp; 3. 手机收到新密码</strong>
				</div>
			</div>
 ...
 <div class="input"><label> 您绑定的手机号码为：</label><?php echo substr($r['mobile'],0,3);?>****<?php echo substr($r['mobile'],-3);?>
 <input type="hidden" name="step" value="3">
 <span id="mobile_div"><button onclick="get_verify()" type="button" class="button">获取短信校验码</button></span>
 <span id="mobile_send" style="display:none">，短信已发送成功，请查收，短信如在10分钟内没有收到，请重试其他方法</span>
 </div>
<script type="text/javascript">
<!--
	function get_verify() {
		$.get("api.php?op=sms",{ session_code: '<?php echo $_SESSION['code'];?>',random:Math.random()}, function(data){
			if(data=="0") {
				$("#mobile_div").hide();
				$("#mobile_send").show();
			} else if(data=="-1") {
...
$(function(){
	$.formValidator.initConfig({autotip:true,formid:"myform",onerror:function(msg){}});
	$("#mobile_verify").formValidator({onshow:"短信校验码",onfocus:"应该为4-8位之间"}).inputValidator({min:4,max:8,onerror:"应该为4-8位之间"});
});
//-->
</script>
