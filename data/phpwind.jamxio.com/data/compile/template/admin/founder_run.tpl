...
<form id="J_founder_form" action="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?c=founder&a=add'; ?>" method="post">
	<table width="100%">
		<col class="th" />
		<col />
<?php  foreach($list as $user){ ?>
		<tr>
			...
			<td>
			<a class="J_dialog mr5" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?username=', rawurlencode($user),'&c=founder&a=edit'; ?>" title="编辑">[编辑]</a>
			<a class="J_ajax_del" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?c=founder&a=del'; ?>" data-pdata="{'username': '<?php echo htmlspecialchars($user, ENT_QUOTES, 'UTF-8');?>'}">[取消创始人权限]</a></td>
		</tr>
<?php  } ?>
	...
	$('#J_founder_form').ajaxForm({
		...
		success : function(data, statusText, xhr, $form) {
			...
			if(data.state == 'success') {
				Wind.dialog.open( decodeURIComponent(data.referer) ,{
					onClose : function() {
