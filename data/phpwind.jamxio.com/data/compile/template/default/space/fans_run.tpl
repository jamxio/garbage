		...
		<div class="space_content">
			<div class="box">
				<div class="my_article">
					<div class="hd"><h2><?php echo htmlspecialchars($host, ENT_QUOTES, 'UTF-8');?>的粉丝</h2></div>
				</div>
				<div class="space_fans">
				<?php  if ($fans) { 
  foreach ($fans as $key => $value) { ?>
					<dl class="cc">
						<dt><a data-uid="<?php echo htmlspecialchars($value['uid'], ENT_QUOTES, 'UTF-8');?>" class="J_user_card_show" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=space&uid=', rawurlencode($value['uid']); ?>"><img class="J_avatar" src="<?php echo htmlspecialchars(Pw::getAvatar($value['uid'], 'small'), ENT_QUOTES, 'UTF-8');?>" data-type="small" width="50" height="50" /></a><a rel="nofollow" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=message&c=message&a=pop&username=', rawurlencode($value['username']); ?>" data-name="<?php echo htmlspecialchars($value['username'], ENT_QUOTES, 'UTF-8');?>" class="called J_send_msg_pop J_qlogin_trigger">打招呼</a></dt>
						<dd>
							<div class="title">
								<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=space&uid=', rawurlencode($value['uid']); ?>" data-uid="<?php echo htmlspecialchars($value['uid'], ENT_QUOTES, 'UTF-8');?>" class="name J_user_card_show"><?php echo htmlspecialchars($value['username'], ENT_QUOTES, 'UTF-8');?></a>
							...
							<div class="num">
								关注<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=space&c=follows&uid=', rawurlencode($value['uid']); ?>"><?php echo htmlspecialchars($value['follows'], ENT_QUOTES, 'UTF-8');?></a><span>|</span>粉丝<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=space&c=fans&uid=', rawurlencode($value['uid']); ?>"><?php echo htmlspecialchars($value['fans'], ENT_QUOTES, 'UTF-8');?></a><span>|</span>帖子<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=space&c=thread&uid=', rawurlencode($value['uid']); ?>"><?php echo htmlspecialchars($value['postnum'], ENT_QUOTES, 'UTF-8');?></a>
							</div>
							...
							<div class="attribute">
								...
								?>
									<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=my&c=follow&a=add'; ?>" class="core_follow J_space_fans J_qlogin_trigger" data-followed="<?php echo htmlspecialchars($isfan, ENT_QUOTES, 'UTF-8');?>" data-uid="<?php echo htmlspecialchars($value['uid'], ENT_QUOTES, 'UTF-8');?>">加关注</a>
								<?php  } ?>
...
?>
	<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=space&c=fans&uid=', rawurlencode($space->spaceUid),'&page=', rawurlencode($_page_i); 
...
<?php } 
  ...
  if ($loginUser->uid == $space->spaceUid) { ?>
					<!--看自己-->
					<div class="not_content J_check_wrap">啊哦，你还没有粉丝，试试关注几个争取点人气吧！</div>
					<?php  } else { ?>
					<!--看别人-->
					<div class="not_content">啊哦，Ta还没有粉丝，我来第一个<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=my&c=follow&a=add'; ?>" id="J_nofans_follow" data-uid="<?php echo htmlspecialchars($space->spaceUid, ENT_QUOTES, 'UTF-8');?>">关注Ta</a>！</div>
					<?php  } 
  } ?>
				</div>
			</div>
