...
<div class="tac"> </div>
	<div class="main_wrap">
		...
		<div class="bread_crumb">
			<?php echo $headguide;?><em>&gt;</em><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?c=user&fid=', rawurlencode($fid); ?>">会员</a>
		</div>
		<div class="main cc">
			<div class="main_body">
 ...
 echo htmlspecialchars($_urladd_, ENT_QUOTES, 'UTF-8');?>" data-referer="true" class="<?php echo htmlspecialchars(trim($postNeedLogin), ENT_QUOTES, 'UTF-8');?>"><?php echo htmlspecialchars($value[0], ENT_QUOTES, 'UTF-8');?></a></li>
					...
					<nav>
						<div class="content_nav" id="hashpos_ttype">
							<ul class="cc">
								<li><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?c=thread&fid=', rawurlencode($fid); ?>">全部</a></li>
								<li><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?c=thread&fid=', rawurlencode($fid),'&tab=digest'; ?>">精华</a></li>
								<li class="current"><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?c=user&fid=', rawurlencode($fid); ?>">会员</a></li>
							</ul>
						</div>
					</nav>
						<div class="thread_uesr_list">
							<h2><span>共<em id="J_user_count"><?php echo htmlspecialchars($totalJoin, ENT_QUOTES, 'UTF-8');?></em>个会员</span>最新加入本版块的会员</h2>
							<?php  if ($joinUser) { ?>
							<ul class="cc" id="J_add_list">
								<?php  foreach ($joinUser as $key => $value) { ?>
								<li id="J_user_<?php echo htmlspecialchars($key, ENT_QUOTES, 'UTF-8');?>">
									<a class="J_user_card_show" data-uid="<?php echo htmlspecialchars($key, ENT_QUOTES, 'UTF-8');?>" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=space&uid=', rawurlencode($key); ?>"><img class="J_avatar" src="<?php echo htmlspecialchars(Pw::getAvatar($key, 'middle'), ENT_QUOTES, 'UTF-8');?>" data-type="middle" width="90" height="90" alt="<?php echo htmlspecialchars($users[$key]['username'], ENT_QUOTES, 'UTF-8');?>" /></a>
									<p><a class="J_user_card_show" data-uid="<?php echo htmlspecialchars($key, ENT_QUOTES, 'UTF-8');?>" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=space&uid=', rawurlencode($key); ?>"><?php echo htmlspecialchars($users[$key]['username'], ENT_QUOTES, 'UTF-8');?></a></p>
								</li>
								<?php  } ?>
							</ul>
							<div id="J_add_none" class="not_content" style="display:none;">啊哦，暂没有会员加入该版块哦，我要<a class="J_qlogin_trigger" id="J_forum_join_trigger" target="_blank" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?c=forum&a=join&fid=', rawurlencode($fid); ?>">加入</a>！</div>
							<?php  } else { ?>
							<ul class="cc" id="J_add_list" style="display:none;"></ul>
							<div id="J_add_none" class="not_content">啊哦，暂没有会员加入该版块哦，我要<a id="J_forum_join_trigger" class="J_qlogin_trigger" target="_blank" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?c=forum&a=join&fid=', rawurlencode($fid); ?>">加入</a>！</div>
							<?php  } ?>
							<h2><span>前<?php echo htmlspecialchars(count($activeUser), ENT_QUOTES, 'UTF-8');?>名</span>本周最活跃用户</h2>
							<?php  if ($activeUser) { ?>
							<ul class="cc">
								<?php  foreach ($activeUser as $key => $value) { if (!isset($users[$key])) continue;?>
								<li>
									<a class="J_user_card_show" data-uid="<?php echo htmlspecialchars($key, ENT_QUOTES, 'UTF-8');?>" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=space&uid=', rawurlencode($key); ?>"><img class="J_avatar" src="<?php echo htmlspecialchars(Pw::getAvatar($key, 'middle'), ENT_QUOTES, 'UTF-8');?>" data-type="middle" width="90" height="90" alt="<?php echo htmlspecialchars($users[$key]['username'], ENT_QUOTES, 'UTF-8');?>" /></a>
									<p><a class="J_user_card_show" data-uid="<?php echo htmlspecialchars($key, ENT_QUOTES, 'UTF-8');?>" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=space&uid=', rawurlencode($key); ?>"><?php echo htmlspecialchars($users[$key]['username'], ENT_QUOTES, 'UTF-8');?></a></p>
								</li>
