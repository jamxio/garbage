		...
		<div class="box_wrap register cc">
			...
			<div class="reg_cont_wrap">
				<div class="reg_cont">
					<div class="reg_form">
						...
						<form id="J_findpw_form" action="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=u&c=findPwd&a=checkUsername'; ?>" method="post">
							...
							<dl>
								<dt>&nbsp;</dt>
								<dd><button class="btn btn_big btn_submit mr20" type="submit">下一步</button><!--a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=u&c=findPwd'; ?>" class="s4">返回上一步</a--></dd>
							</dl>
						<input type="hidden" name="csrf_token" value="<?php echo WindSecurity::escapeHTML(Wind::getComponent('windToken')->saveToken('csrf_token')); ?>"/></form>
					</div>
				</div>
			...
			<div class="reg_side">
				<div class="reg_side_cont">
					...
					<p><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=u&c=login'; ?>" class="btn btn_big">立即登录</a></p>
				</div>
			</div>
		</div>
	</div>
	...
	$("#J_findpw_form").validate({
		...
		rules: {
			username: {
				required	: true,
				remote : {
					url : '<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=u&c=findPwd&a=checkUsername'; ?>',
					dataType: "json",
					type : 'post',
					data : {
						username :  function(){
