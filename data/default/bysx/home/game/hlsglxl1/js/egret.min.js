...
link.imax = 0;
link.debug = false;
link.to = function(m, l, j) {
	var g = [];
	j ? this.matrix = j : j = this.matrix;
	this.imax = j.length;
	this.jmax = j[0].length;
	if (j.length == 0 || j == []) {
		throw "do not has matrix!"
	}
	var n;
	if (m.i == l.i) {
		if (link.debug) {
		...
		if (link.check.parallel(m, l, false)) {
			g = [m, l];
			if (link.debug) {
				console.log("处于同一竖排，连通", g)
			}
			return g
		} else {
			n = link.check.lineByline(m, l, false, true, true);
			if (n) {
				g = [m,
				{
					i: m.i + n,
		...
		if (m.j == l.j) {
			...
			if (link.check.parallel(m, l, true)) {
				g = [m, l];
				if (link.debug) {
					console.log("处于同一横排，连通", g)
				}
				return g
			} else {
				n = link.check.lineByline(m, l, true, true, true);
				if (n) {
					g = [m,
					{
						i: m.i,
			...
			if ((link.matrix[k.i][k.j] == null || link.matrix[k.i][k.j] == 0) && link.check.parallel(m, k, false) && link.check.parallel(l, k, true)) {
				g = [m, k, l];
				if (link.debug) {
					console.log("1折", g)
				}
				...
				if ((link.matrix[i.i][i.j] == null || link.matrix[i.i][i.j] == 0) && link.check.parallel(m, i, true) && link.check.parallel(l, i, false)) {
					g = [m, i, l];
					if (link.debug) {
						console.log("1折", g)
					}
			...
			n = link.check.innerLine(m, l, true, h);
			if (n) {
				g = [m,
				{
					i: m.i + n,
			...
			n = link.check.innerLine(m, l, false, f);
			if (n) {
				g = [m,
				{
					i: m.i,
			...
			if ((link.matrix[k.i][k.j] == null || link.matrix[k.i][k.j] == 0) && link.check.parallel(l, k, true)) {
				d = h ? link.check.lineByline(m, k, false, false, true) : link.check.lineByline(m, k, false, true, false)
			}
			if (link.debug) {
				console.log("从p1点垂直向外逐行检查，返回能连通的行数")
			}
			if ((link.matrix[i.i][i.j] == null || link.matrix[i.i][i.j] == 0) && link.check.parallel(l, i, false)) {
				b = f ? link.check.lineByline(m, i, true, false, true) : link.check.lineByline(m, i, true, true, false)
			}
			if (link.debug) {
				console.log("从p2点垂直向外逐行检查，返回能连通的行数")
			}
			if ((link.matrix[k.i][k.j] == null || link.matrix[k.i][k.j] == 0) && link.check.parallel(m, k, false)) {
				a = f ? link.check.lineByline(l, k, true, true, false) : link.check.lineByline(l, k, true, false, true)
			}
			if (link.debug) {
				console.log("从p1点水平向外逐行检查，返回能连通的行数")
			}
			if ((link.matrix[i.i][i.j] == null || link.matrix[i.i][i.j] == 0) && link.check.parallel(m, i, true)) {
				c = h ? link.check.lineByline(l, i, false, true, false) : link.check.lineByline(l, i, false, false, true)
			}
			var e = 100;
			if (d != 0 && e > Math.abs(d)) {
				e = Math.abs(d)
...
link.check.parallel = function(f, e, b) {
	var d = 0;
	var a = 0;
	var c = 0;
	if (b) {
...
link.check.lineByline = function(h, g, a, c, e) {
	var f = 1;
	var d = {};
	var b = {};
	if (link.debug) {
	...
	if (a) {
		...
		while (h.j - f >= 0 || h.j + f < link.jmax) {
			if (!e && !c) {
				break
			}
			if (e) {
				if (h.j - f >= 0 && (link.matrix[h.i][h.j - f] == null) && (link.matrix[g.i][g.j - f] == null)) {
					d = {
						i: h.i,
						j: h.j - f
					};
					...
					if (link.check.parallel(d, b, true)) {
						return -f
					}
				} else {
					e = false
			...
			if (c) {
				if (h.j + f < link.jmax && (link.matrix[h.i][h.j + f] == null) && (link.matrix[g.i][g.j + f] == null)) {
					d = {
						i: h.i,
						j: h.j + f
					};
		...
		while (h.i - f >= 0 || h.i + f < link.imax) {
			if (!e && !c) {
				break
			}
			if (e) {
				if (h.i - f >= 0 && (link.matrix[h.i - f][h.j] == null) && (link.matrix[g.i - f][g.j] == null)) {
					d = {
						i: h.i - f,
						j: h.j
					};
					...
					if (link.check.parallel(d, b, false)) {
						return -f
					}
				} else {
					e = false
			...
			if (c) {
				if (h.i + f < link.imax && (link.matrix[h.i + f][h.j] == null) && (link.matrix[g.i + f][g.j] == null)) {
					d = {
						i: h.i + f,
						j: h.j
					};
...
link.check.innerLine = function(h, g, e, d) {
	var c = d ? 1 : -1;
	var f = true;
	var j = true;
	var i = 0;
	...
	while (true) {
		if (f) {
			i = b + c;
			if ((e ? link.matrix[i][h.j] : link.matrix[h.i][i]) == null) {
				b = i;
				if (b == a) {
					if (link.check.parallel(e ? {
						i: b,
						j: h.j
					} : {
						i: h.i,
		...
		if (j) {
			i = a - c;
			if ((e ? link.matrix[i][g.j] : link.matrix[g.i][i]) == null) {
				a = i;
				if (b == a) {
					if (link.check.parallel(e ? {
						i: a,
...
function(t) {
	var e = [],
		i = function(i) {
			...
			}, s.dispatchEventWith = function(e, i, n) {
...
function(t) {
	...
	var i = function(i) {
			...
			return a.$setFlags = function(t) {
				...
				if (this.$hasFlags(16)) {
					...
					var i = this.$DisplayObject,
						...
						r = i[13],
						s = this.$scrollRect;
					s ? e.$preMultiplyInto(t.$TempMatrix.setTo(1, 0, 0, 1, -s.x - n, -s.y - r), e) : (0 != n || 0 != r) && e.$preMultiplyInto(t.$TempMatrix.setTo(1, 0, 0, 1, -n, -r), e), this.$displayList && (this.$displayList.$renderRegion.moved = !0), this.$renderRegion && (this.$renderRegion.moved = !0), this.$removeFlags(16)
				}
				return e
			...
			}, a.$render = function(t) {}, a.$hitTest = function(t, e) {
				var i = this.$DisplayObject;
				...
				if (n) {
					var r = this.$getInvertedConcatenatedMatrix(),
						s = r.a * e + r.c * i + r.tx,
						a = r.b * e + r.d * i + r.ty,
						o = t.Rectangle.create();
					o.setTo(s, a, 3, 3);
					var h = new t.RenderTexture;
					h.drawToTexture(this, o);
					var l, c = h.context;
					try {
						l = c.getImageData(0, 0, 1, 1).data
					} catch (u) {
						throw new Error(t.sys.tr(1040))
					}
					var d = !0;
					return 0 === l[3] && (d = !1), t.Rectangle.release(o), h.dispose(), d
				}
				var f = this.$DisplayObject;
				if (0 == f[0] || 0 == f[1]) return !1;
				var r = this.$getInvertedConcatenatedMatrix(),
					g = this.getBounds(null, !1),
					s = r.a * e + r.c * i + r.tx,
					a = r.b * e + r.d * i + r.ty;
				if (g.contains(s, a)) {
					var p = this.$scrollRect ? this.$scrollRect : this.$maskRect;
					return p && !p.contains(s, a) ? !1 : !0
				}
				return !1
...
function(t) {
	var e = function(e) {
			function i(n) {
				e.call(this), this.$scale9Grid = null, this.$fillMode = "scale", this._pixelHitTest = !1, this.$renderRegion = new t.sys.Region, this.$Bitmap = {
					0: null,
					1: null,
					2: 0,
					3: 0,
			...
			return s.$onAddToStage = function(i, n) {
				...
				var r = this.$Bitmap[0];
				r && t.Texture.$addDisplayObject(this, r)
			}, s.$onRemoveFromStage = function() {
				e.prototype.$onRemoveFromStage.call(this);
				var i = this.$Bitmap[0];
				i && t.Texture.$removeDisplayObject(this, i)
				...
				var i = this.$Bitmap,
					n = i[0];
				return e == n ? !1 : (i[0] = e, e ? (this.$refreshImageData(), this.$stage && (n && t.Texture.$removeDisplayObject(this, n), t.Texture.$addDisplayObject(this, e)), this.$invalidateContentBounds(), !0) : (this.setImageData(null, 0, 0, 0, 0, 0, 0, 0, 0), this.$invalidateContentBounds(), !0))
			...
			}, s.setImageData = function(t, e, i, n, r, s, a, o, h) {
				var l = this.$Bitmap;
				...
				if (e[1]) {
					...
					i.$drawImage(t, e[1], e[2], e[3], e[4], e[5], e[6], e[7], e[8], e[9], n, r, this.scale9Grid || e[0].scale9Grid, this.fillMode, e[10])
				}
				...
				if (h) {
					n = h.renderContext;
					try {
						r = n.getImageData(a - h.offsetX, o - h.offsetY, 1, 1).data
					} catch (l) {
						throw console.log(this.$Bitmap[0]), new Error(t.sys.tr(1039))
					}
				} else {
					n = t.sys.hitTestRenderContext, n.surface.width = n.surface.height = 3, n.translate(1 - a, 1 - o), this.$render(n);
					try {
						r = n.getImageData(1, 1, 1, 1).data
					} catch (l) {
						throw console.log(this.$Bitmap[0]), new Error(t.sys.tr(1039))
					}
				}
				return 0 === r[3] ? null : this
			}, i.$drawImage = function(e, n, r, s, a, o, h, l, c, u, d, f, g, p, $) {
				if (n) if (e.imageSmoothingEnabled = $, g) i.$drawScale9GridImage(e, n, g, r, s, a, o, h, l, c, u, d, f);
				else if (p == t.BitmapFillMode.SCALE) {
					var v = d / c,
						y = f / u;
					e.drawImage(n, r, s, a, o, h * v, l * y, v * a, y * o)
				} else if (p == t.BitmapFillMode.CLIP) {
					var x = Math.min(c, d),
						m = Math.min(u, f);
					...
					e.beginPath(), e.rect(0, 0, d, f), e.fillStyle = b, e.fill(), T && t.sys.surfaceFactory.release(T)
				}
			}, i.$drawScale9GridImage = function(e, i, n, r, s, a, o, h, l, c, u, d, f) {
				var g = a,
					p = o;
				d -= c - a * t.$TextureScaleFactor, f -= u - o * t.$TextureScaleFactor;
				var $ = n.x - h,
					v = n.y - l,
				...
				if ((y + F) * t.$TextureScaleFactor > d || (x + w) * t.$TextureScaleFactor > f) return void e.drawImage(i, r, s, a, o, h, l, d, f);
				var D = h,
					M = D + $,
					I = D + (d - L),
					A = d - $ - L,
					...
					P = B + f - O,
					k = f - v - O;
				x > 0 && (y > 0 && e.drawImage(i, _, E, y, x, D, B, $, v), m > 0 && e.drawImage(i, b, E, m, x, M, B, A, v), F > 0 && e.drawImage(i, C, E, F, x, I, B, L, v)), T > 0 && (y > 0 && e.drawImage(i, _, R, y, T, D, N, $, k), m > 0 && e.drawImage(i, b, R, m, T, M, N, A, k), F > 0 && e.drawImage(i, C, R, F, T, I, N, L, k)), w > 0 && (y > 0 && e.drawImage(i, _, S, y, w, D, P, $, O), m > 0 && e.drawImage(i, b, S, m, w, M, P, A, O), F > 0 && e.drawImage(i, C, S, F, w, I, P, L, O))
...
function(t) {
	var e = function() {
			...
			var e = t;
			e.prototype;
			return t.REPEAT = "repeat", t.SCALE = "scale", t.CLIP = "clip", t
		}();
...
function(t) {
	var e = function(e) {
			...
			var n = __define,
				r = i,
				s = r.prototype;
			return n(s, "graphicsRenderContext", function() {
			...
			}, s._parseColor = function(t, e) {
				var i = 255 & t,
					n = (65280 & t) >> 8,
					r = t >> 16;
				return "rgba(" + r + "," + n + "," + i + "," + e + ")"
			}, s._setStyle = function(t) {
			...
			}, s.getGradient = function(e, i, n, r, s) {
			...
			}, s.drawArc = function(t, e, i, n, r, s) {
			...
			}, s._createEndFillCommand = function() {
			...
			}, s._createEndLineCommand = function() {
	...
	var s = Math.PI,
		...
		d = function(d) {
			...
			var g = __define,
				p = f,
				$ = p.prototype;
			return f.createRadialGradient = function(e, i, n, r, s, a) {
				...
				this._lineWidth = t, isNaN(t) ? this._maxLineWidth = 0 : this._maxLineWidth = Math.max(this._maxLineWidth, t), this.pushCommand(3, arguments)
			}), g($, "lineCap", function() {
				...
				var c = Math.cos(r) * n,
					...
					$ = Math.min(g, p),
					v = Math.max(g, p);
				s >= r && l >= s && (d = -n), h >= r && l >= h && (f = n), o >= r && l >= o && ($ = -n), a >= r && l >= a && (v = n), this.extendByPoint(d + t, $ + i), this.extendByPoint(f + t, v + i)
			...
			}, $.$measureContentBounds = function(t) {
				...
				this._strokeStyle && this._maxLineWidth > 0 && (i[15].apply(t, []), i[10].apply(t, [])), this._fillStyle && (i[9].apply(t, []), i[10].apply(t, [])), t.restore()
			}, f
		}(t.HashObject);
...
function(t) {
	...
	var e = function(e) {
			...
			return n(s, "textureWidth", function() {
			...
			}, s.$initData = function(e, n, r, s, a, o, h, l, c, u) {
				var d = t.$TextureScaleFactor;
				this._bitmapX = e / d, this._bitmapY = n / d, this._bitmapWidth = r / d, this._bitmapHeight = s / d, this._offsetX = a, this._offsetY = o, this._textureWidth = h, this._textureHeight = l, this._sourceWidth = c, this._sourceHeight = u, i.$invalidate(this)
				...
				var n;
				if (e instanceof i ? e._bitmapData && (n = e._bitmapData.hashCode) : n = e.hashCode, n && i._displayList[n]) for (var r = i._displayList[n], s = 0; s < r.length; s++) r[s] instanceof t.Bitmap && r[s].$refreshImageData(), r[s].$invalidateContentBounds()
...
function(t) {
	var e = ["source-over", "lighter", "destination-out"],
		...
		n = function(n) {
			...
			var s = r,
				a = s.prototype;
			return a.drawToTexture = function(e, i, n) {
				if (void 0 === n && (n = 1), i && (0 == i.width || 0 == i.height)) return !1;
				this.rootDisplayList && (t.sys.DisplayList.release(this.rootDisplayList), this.rootDisplayList = null), this.context && t.sys.surfaceFactory.release(this.context.surface);
			...
			}, a.$saveParentDisplayList = function(e) {
			...
			}, a.drawDisplayObject = function(t, e, i) {
				var n, r, s = 0;
				...
				if (u) {
					var f = u.$getOriginalBounds();
					l = t.sys.Region.create(), l.updateRegion(f, u.$getConcatenatedMatrix())
				}
				var g;
				if (c && (g = t.sys.Region.create(), g.updateRegion(c, d)), g && l ? (g.intersect(l), t.sys.Region.release(l)) : !g && l && (g = l), g) {
					if (g.isEmpty()) return t.sys.Region.release(g), a
				} else g = t.sys.Region.create(), f = n.$getOriginalBounds(), g.updateRegion(f, n.$getConcatenatedMatrix());
				var p = this.createRenderContext(g.width, g.height);
				if (!p) return a += this.drawDisplayObject(n, r, s), t.sys.Region.release(g), a;
				if (c) {
					var $ = d;
					p.setTransform($.a, $.b, $.c, $.d, $.tx - g.minX, $.ty - g.minY), p.beginPath(), p.rect(c.x, c.y, c.width, c.height), p.clip()
				}
				p.setTransform(1, 0, 0, 1, -g.minX, -g.minY);
				var v = t.Matrix.create().setTo(1, 0, 0, 1, -g.minX, -g.minY);
				if (a += this.drawDisplayObject(n, p, v), t.Matrix.release(v), u) {
					var y = this.createRenderContext(g.width, g.height);
					if (!y) return a += this.drawDisplayObject(n, r, s), t.sys.surfaceFactory.release(p.surface), t.sys.Region.release(g), a;
					y.setTransform(1, 0, 0, 1, -g.minX, -g.minY), v = t.Matrix.create().setTo(1, 0, 0, 1, -g.minX, -g.minY);
					var x = this.drawDisplayObject(u, y, v);
					t.Matrix.release(v), x > 0 && (a += x, p.globalCompositeOperation = "destination-in", p.setTransform(1, 0, 0, 1, 0, 0), p.globalAlpha = 1, p.drawImage(y.surface, 0, 0)), t.sys.surfaceFactory.release(y.surface)
				}
				return a > 0 && (a++, o && (r.globalCompositeOperation = h), s ? (r.translate(g.minX, g.minY), r.drawImage(p.surface, 0, 0), r.setTransform(s.a, s.b, s.c, s.d, s.tx, s.ty)) : (r.setTransform(1, 0, 0, 1, g.minX, g.minY), r.drawImage(p.surface, 0, 0)), o && (r.globalCompositeOperation = i)), t.sys.surfaceFactory.release(p.surface), t.sys.Region.release(g), a
			}, a.drawWithScrollRect = function(e, i, n) {
				var r = 0,
			...
			}, a.createRenderContext = function(e, i) {
...
function(t) {
	var e = function(e) {
			...
			}, s.registerImplementation = function(t, e) {
				this.implMap[t] = e
			}, s.getImplementation = function(t) {
...
function(t) {
	...
	var n = Math.PI,
		...
		l = function(r) {
			...
			return s.release = function(t) {
			...
			}, c.prepend = function(t, e, i, n, r, s) {
				var a = this.tx;
				if (1 != t || 0 != e || 0 != i || 1 != n) {
					var o = this.a,
						h = this.c;
			...
			}, c.deltaTransformPoint = function(e) {
				var i = this,
					n = i.a * e.x + i.c * e.y,
					r = i.b * e.x + i.d * e.y;
			...
			}, c.createBox = function(e, i, n, r, s) {
				...
				var e = this.a,
					...
					y = i * o + r * c + a,
					x = 0;
				u > f && (x = u, u = f, f = x), p > v && (x = p, p = v, v = x), t.x = Math.floor(p > u ? u : p), t.width = Math.ceil((f > v ? f : v) - t.x), d > g && (x = d, d = g, g = x), $ > y && (x = $, $ = y, y = x), t.y = Math.floor($ > d ? d : $), t.height = Math.ceil((g > y ? g : y) - t.y)
			}, c.getDeterminant = function() {
			...
			}, c.$updateScaleAndRotation = function(t, n, r, s) {
			...
			}, c.$preMultiplyInto = function(t, e) {
				var i = t.a * this.a,
					n = 0,
					r = 0,
					s = t.d * this.d,
...
function(t) {
	...
	function(e) {
		var i = [],
			...
			s = function(s) {
				function a(i) {
					s.call(this), this.$isDirty = !1, this.$renderAlpha = 1, this.$renderMatrix = new t.Matrix, this.$ratioMatrix = new t.Matrix, this.$ratioChanged = !1, this.$pixelRatio = 1, this.$renderRegion = new e.Region, this.surface = null, this.offsetX = 0, this.offsetY = 0, this.needRedraw = !1, this.needUpdateRegions = !1, this.rootMatrix = new t.Matrix, this.dirtyNodes = {}, this.dirtyNodeList = [], this.dirtyList = null, this.dirtyRegion = new e.DirtyRegion, this.sizeChanged = !1, this.root = i, this.dirtyRegion.displayList = this
				}
				__extends(a, s);
				var o = a,
					h = o.prototype;
				return a.release = function(n) {
					...
					if (this.needUpdateRegions && this.updateDirtyRegions(), !n) return s.setTo(0, 0, 0, 0), s.moved = !1, !1;
				...
				}, h.drawToSurface = function() {
					var t = 0,
						e = this.dirtyList;
					if (e && e.length > 0) {
						var i = this.rootMatrix;
						i && this.changeSurfaceSize();
						var n = this.renderContext;
						n.save(), n.beginPath(), i && n.setTransform(1, 0, 0, 1, -this.offsetX * this.$pixelRatio, -this.offsetY * this.$pixelRatio);
						for (var r = e.length, s = 0; r > s; s++) {
							var a = e[s];
							n.clearRect(a.minX, a.minY, a.width, a.height), n.rect(a.minX, a.minY, a.width, a.height)
						}
						n.clip(), i && n.setTransform(i.a, i.b, i.c, i.d, i.tx, i.ty), t = this.drawDisplayObject(this.root, n, e, i, null, null), n.restore()
					}
					return this.dirtyList = null, this.dirtyRegion.clear(), this.needRedraw = !1, this.$ratioChanged = !1, t
				}, h.drawDisplayObject = function(t, e, i, n, r, s) {
					var a, o, h = 0;
					...
					if (r) return h;
					var f = t.$children;
					if (f) for (var g = f.length, p = 0; g > p; p++) {
						var $ = f[p];
					...
					return h
				}, h.drawWithClip = function(i, s, a, o, h) {
					var l = 0,
						c = 0 !== i.$blendMode;
					if (c) {
						var u = n[i.$blendMode];
					...
					var d = i.$scrollRect ? i.$scrollRect : i.$maskRect,
						f = i.$mask;
					f && !f.$parentDisplayList && (f = null);
					var g, p = t.Matrix.create();
					p.copyFrom(i.$getConcatenatedMatrix());
					var $, v = i.$parentDisplayList.root;
					if (v !== i.$stage && ($ = v.$getInvertedConcatenatedMatrix(), $.$preMultiplyInto(p, p)), this.$ratioMatrix.$preMultiplyInto(p, p), f) {
						var y = f.$getOriginalBounds();
						g = e.Region.create();
						var x = t.Matrix.create();
						x.copyFrom(f.$getConcatenatedMatrix()), $ && $.$preMultiplyInto(x, x), this.$ratioMatrix.$preMultiplyInto(x, x), g.updateRegion(y, x), t.Matrix.release(x)
					}
					var m;
					if (d && (m = e.Region.create(), m.updateRegion(d, p)), m && g ? (m.intersect(g), e.Region.release(g)) : !m && g && (m = g), m) {
						if (m.isEmpty() || h && !h.intersects(m)) return e.Region.release(m), t.Matrix.release(p), l
					} else m = e.Region.create(), y = i.$getOriginalBounds(), m.updateRegion(y, p);
					for (var T = !1, _ = a.length, b = 0; _ > b; b++) if (m.intersects(a[b])) {
						T = !0;
						break
					}
					if (!T) return e.Region.release(m), t.Matrix.release(p), l;
					var C = this.createRenderContext(m.width, m.height);
					if (!C) return l += this.drawDisplayObject(i, s, a, o, i.$displayList, h), e.Region.release(m), t.Matrix.release(p), l;
					if (d) {
						var x = p;
					...
					if (l += this.drawDisplayObject(i, C, a, F, i.$displayList, m), t.Matrix.release(F), f) {
						var E = this.createRenderContext(m.width, m.height);
						if (!E) return l += this.drawDisplayObject(i, s, a, o, i.$displayList, h), e.surfaceFactory.release(C.surface), e.Region.release(m), t.Matrix.release(p), l;
						E.setTransform(1, 0, 0, 1, -m.minX, -m.minY), F = t.Matrix.create().setTo(1, 0, 0, 1, -m.minX, -m.minY);
						var R = this.drawDisplayObject(f, E, a, F, f.$displayList, m);
					...
					return l > 0 && (l++, c && (s.globalCompositeOperation = u), s.globalAlpha = 1, o ? (s.translate(m.minX, m.minY), s.drawImage(C.surface, 0, 0), s.setTransform(o.a, o.b, o.c, o.d, o.tx * this.$pixelRatio, o.ty * this.$pixelRatio)) : (s.setTransform(1, 0, 0, 1, m.minX, m.minY), s.drawImage(C.surface, 0, 0)), c && (s.globalCompositeOperation = r)), e.surfaceFactory.release(C.surface), e.Region.release(m), t.Matrix.release(p), l
				}, h.drawWithScrollRect = function(i, n, r, s, a) {
					var o = 0,
					...
					for (var d = !1, f = r.length, g = 0; f > g; g++) if (u.intersects(r[g])) {
						d = !0;
						break
					}
				...
				}, h.createRenderContext = function(i, n) {
				...
				}, h.setDevicePixelRatio = function(e) {
				...
				}, h.setDirtyRegionPolicy = function(t) {
...
function(t) {
	var e = function() {
			...
			var e = t;
			e.prototype;
			return t.AUTO = "auto", t.PORTRAIT = "portrait", t.LANDSCAPE = "landscape", t.LANDSCAPE_FLIPPED = "landscapeFlipped", t
		}();
...
function(t) {
	...
	function(e) {
		function i(i, n, r, s) {
			...
			}), l = s ? {} : s, n = !! n, this.showFPS = !! i, this.showLog = n, !this.fpsDisplay) {
				var o = void 0 === s.x ? 0 : s.x,
					u = void 0 === s.y ? 0 : s.y;
				h = this.fpsDisplay = new FPS(this.stage, i, n, r, s), h.x = o, h.y = u;
				for (var d = c.length, f = 0; d > f; f++) h.updateInfo(c[f]);
				c = null
			}
		}
		function n(t) {
		...
		var o = function(a) {
				...
				}, l.updateStageSize = function(e, i, n) {
					void 0 === n && (n = 1);
					var r = this.stage;
					(e !== r.$stageWidth || i !== r.$stageHeight || this.screenDisplayList.$pixelRatio !== n) && (r.$stageWidth = e, r.$stageHeight = i, this.screenDisplayList.setDevicePixelRatio(n), this.screenDisplayList.setClipRect(e, i), this.stageDisplayList && (this.stageDisplayList.setDevicePixelRatio(n), this.stageDisplayList.setClipRect(e, i)), r.dispatchEventWith(t.Event.RESIZE), r.$invalidate(!0))
				}, o
			}(t.HashObject);
			...
			n = function() {
				...
				return t.release = function(t) {
					...
					var i, n, r, s, a = e,
						...
						p = f + t.width,
						$ = g + t.height;
					if (1 == o && 0 == h && 0 == l && 1 == c) i = Math.floor(f + u) - 1, n = Math.floor(g + d) - 1, r = Math.ceil(p + u) + 1, s = Math.ceil($ + d) + 1;
					else {
						var v = o * f + l * g + u,
							y = h * f + c * g + d,
							x = o * p + l * g + u,
	...
	var e = function(e) {
			...
			return n(s, "smoothing", function() {
				...
				var n = this,
					...
					s = n.$getTextLines(),
					a = s.length;
				if (0 != a) for (var o = n.$textLinesWidth, h = r[5], l = h._getFirstCharHeight(), c = Math.ceil(l * i.EMPTY_FACTOR), u = !isNaN(r[1]), d = r[8], f = r[0], g = r[1], p = r[10], $ = n.$textOffsetY + n.$textStartY, v = n.$lineHeights, y = 0; a > y; y++) {
					var x = v[y];
					if (u && y > 0 && $ + x > g) break;
					var m = s[y],
						T = m.length,
						_ = n.$textOffsetX;
					if (p != t.HorizontalAlign.LEFT) {
						var b = f > d ? f : d;
						p == t.HorizontalAlign.RIGHT ? _ += b - o[y] : p == t.HorizontalAlign.CENTER && (_ += Math.floor((b - o[y]) / 2))
					}
					e.imageSmoothingEnabled = r[12];
					for (var C = 0; T > C; C++) {
						var F = m.charAt(C),
							E = h.getTexture(F);
						if (E) {
				...
				for (var h = r[3], l = r[4], c = 0, u = 0, d = 0, f = 0, g = !isNaN(r[0]), p = r[0], $ = r[1], v = r[5], y = v._getFirstCharHeight(), x = Math.ceil(y * i.EMPTY_FACTOR), m = r[2], T = m.split(/(?:\r\n|\r|\n)/), _ = T.length, b = !0, C = 0; _ > C; C++) {
					for (var F = T[C], E = F.length, R = 0, S = 0, w = !0, L = !1, O = 0; E > O; O++) {
						w || (S += l);
						var D, M, I = F.charAt(O),
							A = 0,
							B = 0,
							N = v.getTexture(I);
						if (N) D = N.$getTextureWidth(), M = N.$getTextureHeight(), A = N._offsetX, B = N._offsetY;
						else {
							if (" " != I) {
								t.$warn(1011, I), w && (w = !1);
								continue
						...
						if (w && (w = !1, d = Math.min(A, d)), b && (b = !1, f = Math.min(B, f)), g && O > 0 && S + D > p) {
							if (!e(F.substring(0, O))) break;
							F = F.substring(O), E = F.length, O = 0, S = D, R = M
						} else S += D, R = Math.max(M, R)
					}
					if ($ && C > 0 && u > $) break;
					if (L = !0, !e(F)) break
				}
				u -= h, r[8] = c, r[9] = u, n.$textOffsetX = d, n.$textOffsetY = f, n.$textStartX = 0, n.$textStartY = 0;
				var P;
				return p > c && (P = r[10], P == t.HorizontalAlign.RIGHT ? n.$textStartX = p - c : P == t.HorizontalAlign.CENTER && (n.$textStartX = Math.floor((p - c) / 2))), $ > u && (P = r[11], P == t.VerticalAlign.BOTTOM ? n.$textStartY = $ - u : P == t.VerticalAlign.MIDDLE && (n.$textStartY = Math.floor(($ - u) / 2))), s
			}, i.EMPTY_FACTOR = .33, i
		}(t.DisplayObject);
...
function(t) {
	var e = function(e) {
			...
			}, r.onMouseDownHandler = function(e) {
				e.stopPropagation();
...
function(t) {
	...
	function i(t, e, i) {
		...
		var n = null == i.italic ? e.$TextField[16] : i.italic,
			...
			h = n ? "italic " : "normal ";
		h += s ? "bold " : "normal ", h += a + "px " + o, t.font = h, t.textAlign = "left", t.textBaseline = "middle", t.lineJoin = "round"
	}
	var n = new RegExp("(?=[\\u00BF-\\u1FFF\\u2C00-\\uD7FF]|\\b|\\s)(?![。，！、》…）)}”】\\.\\,\\!\\?\\]\\:])"),
		r = function(r) {
			...
			return h.isInput = function() {
			...
			}, a(h, "displayAsPassword", function() {
				return this.$TextField[20]
			}, function(t) {
				this.$setDisplayAsPassword(t)
			}), h.$setDisplayAsPassword = function(t) {
				...
				return !1
			}, a(h, "strokeColor", function() {
				return this.$TextField[25]
			}, function(t) {
				this.$setStrokeColor(t)
			}), h.$setStrokeColor = function(e) {
			...
			}, h.$invalidateTextField = function() {
				...
				s > 0 && (n.width += 2 * s, n.height += 2 * s), n.x -= s + 2, n.y -= s + 2, n.width = Math.ceil(n.width) + 4, n.height = Math.ceil(n.height) + 4;
				var a = r.prototype.$update.call(this, n);
				return t.Rectangle.release(n), a
			}, h.$measureContentBounds = function(e) {
				var i = this;
				this.$getLinesArr();
				...
				e.isFlow || i(s, e);
				for (var o, h = e.linesArr, l = 0, c = 0, u = 0, d = 0, f = 0, g = r.length; g > f; f++) {
					var p = r[f];
					p.style = p.style || {};
					for (var $ = p.text.toString(), v = $.split(/(?:\r\n|\r|\n)/), y = 0, x = v.length; x > y; y++) {
						null == h[d] && (o = {
							width: 0,
							height: 0,
							elements: [],
							charNum: 0,
							hasNextLine: !1
						}, h[d] = o, l = 0, u = 0, c = 0), u = this.$TextField[24] == t.TextFieldType.INPUT ? this.$TextField[0] : Math.max(u, p.style.size || this.$TextField[0]);
						var m = !0;
						if ("" == v[y]) y == x - 1 && (m = !1);
						else {
							e.isFlow && i(s, e, p.style);
					...
					f == r.length - 1 && o && (o.width = l, o.height = u, o.charNum = c, this.$TextField[5] = Math.max(this.$TextField[5], l), this.$TextField[6] += u)
				}
				return this.$TextField[29] = h.length, this.fillBackground(), h
			}, h.drawText = function(i) {
				var n = this,
					r = this.$TextField,
					s = n.$getLinesArr();
				if (0 != r[5]) {
					...
					h = Math.round(h);
					for (var d = t.TextFieldUtils.$getHalign(n), f = 0, g = l, p = r[29]; p > g; g++) {
						var $ = s[g],
							v = $.height;
						if (h += v / 2, g != l) {
						...
						f = Math.round((a - $.width) * d);
						for (var y = 0, x = $.elements.length; x > y; y++) {
							var m = $.elements[y],
								T = m.style.size || r[0];
							e(i, n, m.text, f, h + (v - T) / 2, m.width, m.style), m.style.underline && this.drawTempArray.push({
								x: f,
								y: h + v / 2,
								w: m.width,
								c: m.style.textColor
	...
	var e = function() {
			...
			return e.$getStartLine = function(t) {
				...
				for (var c = e.$getStartLine(t), u = 0, d = c; d < r.length; d++) {
					...
					if (u += f.height, u + t.$TextField[1] > n) return null;
					u += t.$TextField[1]
				}
				if (0 == s) return null;
				var g = r[s - 1],
	...
	var i = function() {
			...
			return s._setArrayBuffer = function(t) {
				...
				if (!(0 > e) && !(0 > i) && (n = 0 == i ? t.length - e : Math.min(t.length - e, i), n > 0)) {
					...
					for (var r = new DataView(t.buffer), i = n, s = 4; i > s; i -= s) this.data.setUint32(this._position, r.getUint32(e)), this.position += s, e += s;
					for (; i > 0; i--) this.data.setUint8(this.position++, r.getUint8(e++))
				}
